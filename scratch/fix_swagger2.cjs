...
// Find the index of the line that starts the duplicate block
const startMarker = "# ─────────────────────────────────────────────────────────────────────────";
...
// 3. Check if schemas block is already inserted in components
if (!content.includes('schemas:\n    Payment:')) {
  ...
  // Handle CRLF or LF
  let componentsIndex = content.indexOf('components:\r\n');
  if (componentsIndex === -1) componentsIndex = content.indexOf('components:\n');

  if (componentsIndex !== -1) {
    // Replace components:\r\n with components:\r\n + schemasBlock
    let newline = content.indexOf('\r\n') !== -1 ? '\r\n' : '\n';
    content = content.replace('components:' + newline, 'components:' + newline + schemasBlock + newline);
  }
}
